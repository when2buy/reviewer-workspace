...
|---|---|---|---|---|---|
| #183 | pca-yield-curve | Dongzhikang | Haiku 24/26; `var_explained` written as percentage (86.32) instead of decimal (0.8632); instruction missing scale clarification | ✅ Root cause confirmed. Added 1 sentence to the `eigenvalues.csv` output schema in `instruction.md`: "`var_explained` and `cumulative_var_explained` are decimal fractions between 0 and 1, not percentages." → Haiku 26/26 | ✅ 1-sentence instruction fix |
...
| #195 | creditrisk-plus-model | Dongzhikang | TO:25/30:25/30 | Sonnet/Opus VaR **identical** (5.9/7.7/10.2) but systematically off from oracle (6.5/9.0/12.6); root cause: instruction line 29 contradiction — Var(S_k)=σ_k²/μ_k² conflicts with body defining σ_k as std | ✅ Root cause confirmed. Changed line 29 of `instruction.md` from `Var(S_k) = σ_k² / μ_k²` to `Var(S_k) = σ_k²` (removed the `/μ_k²` term, which incorrectly treated σ_k as a coefficient of variation rather than a standard deviation) → Haiku 30/30 | ✅ 1-line instruction fix |
